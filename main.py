...
import argparse
from core.model import CenterlineClassifier
import json

def get_parser():
    # create parser object
    parser = argparse.ArgumentParser(
        description='Neural network classifier for centerlines using Tensorflow',
...
def main(args):
    ...
    with tf.Session(config=config_proto) as sess:
        model = CenterlineClassifier(sess,config)
        if args.phase == "TRAIN":
            model.train()
        else:
            model.predict() 
